...
@Entity
@Table(name = "KCV_LENDER")
public class Lender implements Serializable{

	private static final long serialVersionUID = 5250518280143341192L;

	@Id
	@GeneratedValue(strategy=GenerationType.TABLE)
	@Column(name = "KCV_LENDER_ID",nullable = false )
	...
	@OneToMany(mappedBy="lenderContact" )
	//@Cascade({CascadeType.PERSIST, CascadeType.SAVE_UPDATE,CascadeType.DELETE})
	@Cascade({CascadeType.ALL})
	private Set<Contact> contacts;

	@OneToMany(mappedBy="lenderUser" )
	//@Cascade({CascadeType.PERSIST, CascadeType.SAVE_UPDATE,CascadeType.DELETE})
	@Cascade({CascadeType.ALL})
	private Set<User> users;

	@OneToMany(mappedBy="lenderCriteria" )
	//@Cascade({CascadeType.PERSIST, CascadeType.SAVE_UPDATE,CascadeType.DELETE})
	@Cascade({CascadeType.ALL})
	private Set<LoanCriteria> criterias;
	...
	public Set<LoanCriteria> getCriterias() {
		return criterias;
	}

	public void setCriterias(Set<LoanCriteria> criterias) {
